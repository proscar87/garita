...
G=5 y se multiplica por 4; los dígitos, por 3-2-7-6-5-4-3-2; módulo 11, y
si 11 menos el residuo pasa de 9, el verificador es 0.
...
_RIF = re.compile(r"(?<![\w-])[VEJPGvejpg]\s?-?\s?\d{8}\s?-?\s?\d(?![\w-])")
_LETRAS = {"V": 1, "E": 2, "J": 3, "P": 4, "G": 5}
...
def rif_valido(v: str) -> bool:
    d = limpio(v)
    m = re.fullmatch(r"([VEJPG])(\d{8})(\d)", d)
    if not m:
        return False
